<template>
  <div>
        ...
        <div class="css-1km3mtt ant-app">
          <div class="relative z-20 h-[187px] md:h-[309px] lg:h-[285px] bg-no-repeat">
            <picture>
              ...
              <source srcset="/images/bitget/banner-bg-md.5e0624e6.svg" media="(min-width: 641px) and (max-width: 768px)">
                  ...
                  <div class="px-[16px] md:px-[24px]">
                    ...
                    <div class="flex justify-center mt-2 gap-2 items-center" v-if="store.state.userInfo?.code">
                      <div class="demo-progress">
                        <el-progress :text-inside="true" :stroke-width="28" :percentage="processNumber"
                          status="success" />
                      </div>
                    </div>
                    <div class="md:relative flex items-center mt-6 md:mt-6 gap-x-[10px] md:gap-x-0 mx-auto max-w-[311px]">
                      <div @click="onPopup"
                    ...
                    <div class="mt-[32px] md:mt-[56px]">
                      <h3 class="flex my-0">
                        <div class="my-[6px] md:my-[2px] w-[3px] bg-[#00FFF1] me-[8px]"></div>
                        <span class="text-primaryText text-xl md:text-[1.75rem] leading-[28px] font-medium">如何獲得獎勵</span>
                      </h3>
                      <div
    ...
    <el-dialog @close="resetFields" destroy-on-close v-model="state.dialog"
      ...
      <el-form ref="ruleFormRef" :model="state.form" :rules="formRules">
        <el-form-item prop="phone">
          <el-input size="large" type="phone" v-model="state.form.phone" placeholder="电话号码"></el-input>
        </el-form-item>
      </el-form>
      <template #footer>
    ...
    <el-dialog destroy-on-close v-model="state.promptDialog"
      class="md:w-[50%] lg:w-[40%] xl:w-[30%] w-[85%] rounded-2xl bg-gradient-to-r from-green-200 to-green-400" center
...
import { reactive, watchEffect, watch, onMounted, computed, onUnmounted, ref } from 'vue';
...
const submitProcess = async () => {
  ...
  try {
    const response = await api.savePhone(requrest);
...
const openWhatsApp = (type: Number = 1, code = '') => {
  const protocol = window.location.protocol;
  const domain = window.location.hostname;
  const port = window.location.port;
  const fullDomain = protocol + '//' + domain + (port ? ':' + port : '');
  var content = "";
  content += "点击下面链接👇👇👇,通过参活动来赚钱吧。";
 
  // state.form.url = (type !== 1 ? 'https://api.whatsapp.com/send' : 'whatsapp://send/') + `?text=${encodeURIComponent(content)}`;
  if (type === 1) {
...
watchEffect(() => {
  if (state.form.click === 10) {
    const setIntverDialog = setInterval(() => {
      state.promptDialog = true;
      clearInterval(setIntverDialog);
    }, 2000);
  }
});
watch(() => store.state?.click, () => {
  if (store.state.click === 1) {
...
const processNumber = ref<number>(0)
const setIntervalFunc = (num: number) => {
  const setIntverDialog = setInterval(() => {
    processNumber.value = num;
      clearInterval(setIntverDialog);
    }, 2000);
}
watchEffect(() => {
